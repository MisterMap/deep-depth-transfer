...
class SpatialPhotometricConsistencyLoss(torch.nn.Module):
    ...
    def forward(self, left_current_img, right_current_img, left_current_depth, right_current_depth):
        device = left_current_depth.device
        generated_right_img = kornia.warp_frame_depth(
            image_src=left_current_img,
            depth_dst=right_current_depth,
            src_trans_dst=torch.inverse(self.transform_from_left_to_right).to(device),
            camera_matrix=self.left_camera_matrix.to(device)
        )

        generated_left_img = kornia.warp_frame_depth(
            image_src=right_current_img,
            depth_dst=left_current_depth,
            src_trans_dst=self.transform_from_left_to_right.to(device),
            camera_matrix=self.right_camera_matrix.to(device)
        )
